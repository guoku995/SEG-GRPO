...
def collate_fn(
    ...
    for (
        ...
        cot_reasoning_level_list.append(cot_reasoning_level)
    ...
    return {
        ...
        "cot_reasoning_level_list": cot_reasoning_level_list,
    }
...
class VALDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        val_dataset,
        processor=None,
        cot_reasoning_level="light",
    ):

        self.processor = processor
        self.base_image_dir = base_image_dir
        self.cot_reasoning_level = cot_reasoning_level
    ...
    def get_reasoning_prompt(self):
        if self.cot_reasoning_level == "light":
            return (
                "Use light object-level reasoning. In <think>, write exactly one section: Observation. "
                "Observation should be one concise sentence describing the target object's visible appearance and coarse image region. "
                "Do not compare candidates, explain relationships, or mention coordinates. "
            )
        if self.cot_reasoning_level == "heavy":
            return (
                "Use heavy context-level reasoning. In <think>, write exactly three sections: Observation, Comparison, and Context reasoning. "
                "Observation should be one concise sentence describing the target object's visible appearance and coarse image region. "
                "Comparison should first mention nearby or similar visible candidates, then explain how the target differs from them using appearance, position, overlap, pose, or proximity. "
                ...
                "content": [
                    ...
                    },
                    {
                        ...
                        "text": QUESTION_TEMPLATE.format(Question=sampled_sents[i].lower().strip("."),
                                                         CoTReasoningPrompt=self.get_reasoning_prompt(),
                                                         #Answer="[SEG]")
