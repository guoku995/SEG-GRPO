...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # #
        # QUESTION_TEMPLATE = \
        #     ("Please observe carefully and analyze what you see that helps you to segment out '{Question}' in the image "
        #      "Output the observing process in <observe> </observe> and final answer in <answer> </answer> tags."
        #      "i.e., <observe> observing process here </observe>" "<answer>{Answer}</answer>" )
        ...
        QUESTION_TEMPLATE = \
            "Find '{Question}'." \
            "The image size is 256*256, locate the most closely matched one." \
            "Directly output the answer with one bbox and the concept name of {Question} in <answer> </answer> tags. i.e., <answer>{Answer}</answer> in JSON format." \


        messages = []
        i = 0
        while i < len(sampled_sents):
            obj_name = sampled_sents[i]
            answer_json = f'{{"bbox": [10,100,200,210], "name": "{obj_name}"}}'
            message = [{
                "role": "user",
                "content": [
                    {
