...
class GRPOConfig(TrainingArguments):
    ...
    reprompt_threshold: float = field(
        default=0.3,
        metadata={"help": "IoU reward threshold used to stop the training-time re-prompting loop."},
    )
    max_reprompt_rounds: int = field(
        default=2,
        metadata={"help": "Maximum number of corrective re-prompt rounds after the initial generation."},
    )
    revision_reward_weight: float = field(
        default=1.0,
        metadata={"help": "Weight for the mask-IoU improvement shaping reward used after re-prompt revision."},
    )
    revision_advantage_weight: float = field(
        default=1.0,
        metadata={"help": "Weight for post-scaling GRPO advantages by the revision improvement signal."},
    )
